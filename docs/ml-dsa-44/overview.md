...
| **1 — User payments**      | A PQ fee payer sends SOL via an ordinary transaction carrying an ML-DSA envelope carrier (re-verified on every peer's replay); RPC validates PQ txs before submission. _Single signer, legacy message only._                                         | Unit + bank-execution + live transfer demo        |
