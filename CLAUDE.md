...
This fork migrates Solana's signatures from **Ed25519 to ML-DSA-44** (NIST FIPS 204, post-quantum) across the **five signing surfaces** a validator uses (app-level verify, user payments, validator votes, node gossip, block broadcasting). Phases 0–3 are **done & verified**. User payments and validator votes now **finalize across a real multi-node (N=2) cluster** via a **replay-safe ML-DSA envelope carrier** — the proof rides inside an ordinary transaction (an ML-DSA precompile instruction), re-verified on every peer's block replay; this replaced the old `0x00`/`MlDsaTransaction` format (now deleted). Every post-quantum path **coexists** with Ed25519 (flag-gated or additive), so the node never stalls. **Operational constraints:** run with GPU sigverify **off** (envelope packets are CPU-only for now), and upgrade **all nodes together** (no runtime feature gate). Phase 4 (flip the node identity itself to an ML-DSA address) is deferred — blocked by the QUIC/TLS Ed25519 cert wall.
